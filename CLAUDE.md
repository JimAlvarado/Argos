...
Dos equipos, mismo código: la PC de planta (20 núcleos, **sin GPU**) y la
laptop G15 de desarrollo (**RTX 3050 Laptop, 4 GB**). El equipo se elige solo
en tiempo de ejecución; no hay ramas ni banderas por máquina.
...
- Antes de dar por terminado CUALQUIER cambio: `python -m unittest discover -s tests -v`
  → deben ser **254 pruebas en verde** (o más si agregaste; nunca menos).
...
- El build de PyTorch decide el equipo, no el código: `+cu130` en la laptop,
  `+cpu` en planta. Comandos exactos en `INSTALAR_WINDOWS_11.md`.
...
- **Fase 2 en curso** — cuatro cámaras del proceso: tolva → horno rotatorio →
  ...
  - Hecho: motor `core/pipeline/estados.py`, tabla `estados` en EventStore y
    ...
    `mantenedor` disponible; `tolva` y `horno` en `available: False`).
  ...
  - **Tolva (cámara 1)**: llenado visible y contrastado (chatarra clara sobre
    interior oscuro). El archivo dice «Tolva norte» y el OSD «Tolva Sur»:
    resolver antes de nombrar la fuente.
